...
void help(trie_node_t *node, char ***result, int i_res, int *i_col,
	  char *searchword, int i_search, int n_search)
{
	...
	for (int i = 0; i < NUM_CHAR; i++) {
		if (node->next[i] != NULL) {
			if (i_search >= n_search ||
			    (int)(searchword[i_search] - 'a') == i) {
				help(node->next[i], result, i_res, i_col,
				     searchword, i_search + 1, n_search);
			}
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	int return_size = 0;
	int *col_size;
	char ***result = suggestedProducts(test_string, LEN, SEARCH,
					   &return_size, &col_size);
